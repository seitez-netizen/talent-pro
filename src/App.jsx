...
// Use environment config to ensure compatibility with the provided auth token
...
// Use the environment's app ID for storage paths to ensure persistence
...
// 画像リサイズ用ユーティリティ (Firestoreの1MB制限対策)
...
const Login = ({ onLogin, onRegister, initialMode = 'login' }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      // エラーハンドリングの改善：設定未完了エラーの場合はconsole.errorを出さない
      if (err.code === 'auth/operation-not-allowed') {
        console.warn("Firebase Auth Configuration Warning: " + err.message);
        setError(
          <div className="text-left">
            <span className="block font-bold mb-1 text-red-600">認証設定が無効です</span>
            <span className="block text-xs mb-3 text-slate-600">
                Firebaseコンソールで「メール/パスワード」ログインが有効になっていません。
                <br/>
                <span className="opacity-75">Authentication &gt; Sign-in method タブで有効にしてください。</span>
            </span>
            <button 
                type="button" 
                onClick={() => onLogin('', '', 'demo')}
                className="w-full py-2 bg-indigo-600 text-white rounded-lg text-xs font-bold hover:bg-indigo-700 transition-colors shadow-sm"
            >
                デモモードで試す (設定不要・データ保存なし)
            </button>
          </div>
        );
      } else {
            ...
            <div className="absolute top-0 right-0 -mt-2 -mr-2 w-20 h-20 bg-white/10 rounded-full blur-xl"></div>
             ...
             <div className="flex items-center space-x-3 mb-4">
                ...
                <div className="overflow-hidden">
                    <p className="text-sm font-bold truncate">{isDemoMode ? 'デモモード' : (user?.isAnonymous ? 'Guest User' : 'User')}</p>
                    <p className="text-[10px] text-slate-400 truncate w-32">
                        {isDemoMode ? '保存されません' : (user?.email || 'データ保存中')}
                    </p>
                </div>
            </div>
...
const TalentForm = ({ talent, onSave, onBack, onDelete, onRetire }) => {
  ...
  const handlePhotoUpload = async (e) => {
    ...
    if (file) {
      try {
        // 画像をリサイズしてからBase64に変換 (Firestoreの制限対策)
        const resizedImage = await resizeImage(file, 800, 0.7);
        
        // サイズチェック (約1MB)
        if (resizedImage.length > 1048000) {
           alert("画像サイズが大きすぎます。別の画像を選択してください。");
           return;
        }
        
        setFormData(prev => ({ ...prev, photo: resizedImage }));
      } catch (error) {
...
const App = () => {
  ...
  // Auth: 自動ログイン機能を追加（トークンがない場合は匿名ログイン）
  useEffect(() => {
    const initAuth = async () => {
        try {
            if (typeof __initial_auth_token !== 'undefined' && __initial_auth_token) {
                ...
                // トークンがない場合、匿名認証で自動サインインを試みる
                // これにより、ログイン画面をスキップし、データ永続化を有効にする
                try {
                  await signInAnonymously(auth);
                } catch (err) {
                   if (err.code === 'auth/operation-not-allowed') {
                       // コンソールで匿名認証が無効になっている場合のログ抑制とハンドリング
                       console.warn("Anonymous auth disabled in Firebase Console. Falling back to Login screen.");
                   } else {
    ...
    if (isDemoMode) {
        ...
        importedData.forEach(item => {
            ...
            if (talent) {
                ...
                // 修正: updateではなくset(merge: true)を使用し、ドキュメントが存在しない場合は作成するように変更
                batch.set(docRef, {
                    ...talent, // 既存のタレント情報を確保
                    rating: item.rating,
  ...
  // ログインしていない場合はログイン画面を表示 (通常は匿名ログインでスキップされます)
